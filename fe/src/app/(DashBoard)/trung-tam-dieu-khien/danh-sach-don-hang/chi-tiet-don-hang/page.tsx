...
export default function Page() {
  ...
  function change_status(
    id_item: { _id: string; code_order: string | number; user_id: string },
    status: number,
    ...
    const message_notification =
      status === 6
        ? `Rất tiếc, người bán đã từ chối đơn hàng ${id_item?.code_order}!.`
        : status === 2
        ? `Đơn hàng ${id_item?.code_order} của bạn đã được xác nhận!.`
        : status === 3
        ? `Đơn hàng ${id_item?.code_order} của bạn đang chuẩn bị giao đến đơn vị vận chuyển!.`
        : status === 4
        ? `Đơn hàng ${id_item?.code_order} đang trên đường vận chuyển tới bạn!.`
        : status_item_order === 6 &&
          `người bán đã chấp nhận yêu cầu hủy đơn hàng ${id_item?.code_order}!.`;
    const data_body = {
      sender_id: user?._id,
    ...
    socket.emit(
      "send_status_item_order_to_user",
      status === 2
        ? `Đơn hàng ${id_item?.code_order} đã được xác nhận, người bán đang chuẩn bị hàng để giao đến bạn`
        : status === 3
        ? `Người bán đang chuẩn bị đơn hàng ${id_item?.code_order} để giao đến bạn!`
        : status === 4
        ? `Đơn hàng ${id_item?.code_order} đang trên đường giao đến bạn!`
        : status === 6 &&
          `Người bán đã từ chối đơn hàng ${id_item?.code_order}, vui lòng chọn sản phẩm khác!`
    );
  }
      ...
      cell: ({ row }) => (
        <div className="flex gap-x-4 lg:gap-x-8">
          ...
          <div className="w-full flex flex-col gap-y-3">
            ...
            {(row?.original?.color_item ||
              row?.original?.size_attribute_item) && (
              <span className="text-sm">
                Phân loại : {row?.original?.color_item} ,{" "}
                {row?.original?.size_attribute_item}
              </span>
            )}
          </div>
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-y-2 text-end">
          <span className="text-red-600">
            {row?.original?.price_item?.toLocaleString("vi", {
              style: "currency",
              currency: "VND",
            })}
          </span>
          ...
          <span className="text-red-600">
            {row?.original?.total_price_item?.toLocaleString("vi", {
              style: "currency",
              currency: "VND",
            })}
          </span>
  ...
  // update status
  function btn_change_status_item_order(
    item: any,
    status: number,
    status_item_order: string | number
  ) {
          ...
          <Button
            className={`${
              status_item_order === 2
                ...
                : status_item_order === 7 && "bg-green-500 hover:!bg-green-700"
            } rounded h-auto text-xs`}
          >
            {status_item_order === 2
              ? "Xác nhận"
              : status_item_order === 6
              ...
              : status_item_order === 4
              ? "Đang vận chuyển"
              : status_item_order === 7 && "Chấp nhận yêu cầu hủy đơn"}
          </Button>
        </AlertDialogTrigger>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {status_item_order === 2
                ? `Xác nhận đơn hàng ${item?.code_order}?`
                : status_item_order === 6
                ? `Từ chối đơn hàng ${item?.code_order}?`
                : status_item_order === 3
                ? `Xác nhận chuẩn bị đơn hàng ${item?.code_order}?`
                : status_item_order === 4
                ? `Xác nhận đơn hàng ${item?.code_order} đang trên đường vận chuyển?`
                : status_item_order === 7 &&
                  `Chấp nhận yêu cầu hủy đơn hàng ${item?.code_order}?`}
            </AlertDialogTitle>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="mt-6 bg-white dark:bg-[#0F1629] p-6 rounded">
        ...
        <div className="text-sm mt-2 pt-6 border-t">
          ...
          <div className="my-2 flex flex-col gap-y-2 *:grid *:grid-cols-[150px_auto]">
            <span>
              Tên: <p>{data?.data_order_by_id?.infor_user?.name_user}</p>
            </span>
            <span>
              Số điện thoại: <p>{data?.data_order_by_id?.infor_user?.phone}</p>
            </span>
            <span>
              Email: <p>{data?.data_order_by_id?.infor_user?.email_user}</p>
            </span>
            <span>
              Địa chỉ: <p>{data?.data_order_by_id?.infor_user?.address}</p>
            </span>
          </div>
        </div>
