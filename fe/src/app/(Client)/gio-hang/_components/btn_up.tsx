...
const Btn_up = ({ id_props }: any) => {
  ...
  function up_quantity(product: any) {
    ...
    const attribute = id_props?.item?.product_id?.variant?.variants?.find(
      (data: any) => data?.attribute === product?.item?.name_varriant
    );
    const value_variants = attribute?.value_variants?.find(
      (value: any) =>
        (value?.name_variant?.trim() ? value?.name_variant : undefined) ===
        product?.item?.value_varriant
    );
