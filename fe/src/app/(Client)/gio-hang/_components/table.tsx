  ...
  return (
    <div>
      ...
      <div className="hidden lg:block ">
        <div
          className="grid lg:grid-cols-[40px_100px_400px_150px_160px_200px_100px] justify-between 
                bg-white p-4 rounded-lg items-center text-gray-700 overflow-x-auto hidden_scroll_y"
        >
          {/* 88 */}
          <Checkbox
            checked={
              dataProps?.data_item_checkked?.length ==
                dataProps?.data?.items?.length &&
          ...
          {dataProps?.data?.items?.length > 0 ? (
            <>
              {dataProps?.data?.items?.map((item: any) => {
                return (
                  <div
                    className="grid lg:grid-cols-[20px_100px_400px_150px_160px_200px_100px] grid-cols-[20px_auto_auto]
                  ...
                  >
                    ...
                    {/* 88 */}
                    <Checkbox
                      checked={item?.status_checked && true}
                      onClick={() =>
                    ...
                    >
                      <Image
                        width={150}
                        height={150}
                        className="relative bg-[#f2f2f2f2] z-[1] rounded w-20 h-20 lg:w-full lg:h-full duration-300"
                    ...
                    <div className="flex flex-col lg:flex-row gap-y-2">
                      <div className="flex flex-col gap-y-2 md:text-base mb:text-xs lg:w-[300px] w-[170px] h-full">
                        ...
                        <section className="hidden lg:flex gap-x-4 items-center *:truncate">
                          <span>{item?.product_id?.seller?.user_name}</span>
                          <Link
                            className="text-sm p-1 underline text-gray-700 rounded flex items-center hover:text-black"
                            href={`cua-hang-truc-tuyen?id=${item?.product_id?.seller?._id}`}
                          >
                      ...
                      <div className="flex flex-col gap-y-2 md:text-base mb:text-xs lg:w-[150px] w-[70px]">
                        {item?.name_varriant && (
                          <div className="flex flex-col text-gray-700">
                            <span className="text-sm mb-1 lg:mb-3">
                              Phân loại :
                            </span>
                    ...
                    {/* 88 */}
                    <span className="hidden lg:block md:text-base mb:text-xs text-red-600">
                    ...
                    <div className="w-[140px] grid grid-cols-3 *:text-gray-900 gap-x-1 items-center justify-around *:md:text-base *:mb:text-xs px-1 rounded-lg *:font-medium">
                      <Btn_dow
                        id_props={{
                          id_item: item?.product_id?._id,
                          quantity_item: item?.quantity,
                          attribute: item?.name_varriant,
                          value_variant: item?.value_varriant,
                          name_item: item?.product_id?.short_name,
                        }}
                      />
                      <strong className="cursor-default border py-2 border-gray-300 grid place-items-center rounded">
                        {item?.quantity}
                      </strong>
                      <Btn_up id_props={{ item: item }} />
                    </div>
                    {/* 88 */}
                    <span className="md:text-base mb:text-xs text-red-600 translate-x-[110px] lg:translate-x-0">
            ...
            <div className="grid place-items-center py-5">
              <div className="flex flex-col items-center gap-y-6 my-auto">
                <Image
                  ...
                  alt="store88"
                />
                <span className="flex flex-col lg:flex-row items-center text-gray-700">
