...
export default function Page_create_seller() {
  ...
  function onSendMessage(dataForm: any) {
    ...
    socket.emit(
      "create_seller_message",
      `Đại vương có 1 thông báo mới từ ${data_user?.user_name}`
    );
  }
  return (
    <div className="pl-4 bg-gray-100 dark:bg-[#0F1629] rounded py-4">
      {loading_user ? (
        ...
        <>
          ...
          <form
            onSubmit={mutate_notification.form_notification.handleSubmit(
              onSendMessage
            )}
          ...
          >
            <div className="flex flex-col gap-y-2 my-4">
              ...
              <input
                ...
                type="text"
                placeholder="Nhập"
                {...mutate_notification.form_notification.register(
                  "name_shop",
                  { required: "Tên shop không được để trống!" }
                )}
                defaultValue={data_user?.user_name}
              />
              {mutate_notification.errors.name_shop && (
                <span className="text-red-500 text-sm">
                  {mutate_notification.errors.name_shop?.message}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-y-2 my-4">
              ...
              />
              {mutate_notification.errors.phone_shop && (
                <span className="text-red-500 text-sm">
                  {mutate_notification.errors.phone_shop?.message}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-y-2 my-4">
              ...
              />
              {mutate_notification.errors.email_shop && (
                <span className="text-red-500 text-sm">
                  {mutate_notification.errors.email_shop?.message}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-y-2 my-4">
              ...
              />
              {mutate_notification.errors.address_shop && (
                <span className="text-red-500 text-sm">
                  {mutate_notification.errors.address_shop?.message}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-y-2 my-4">
            ...
            <div className="flex items-center space-x-2 my-4">
              ...
              >
                Khi đăng kí đăng bán sản phẩm trên sàn Store88 của chúng tôi,
                bạn phải chấp nhận các{" "}
                <Link href={"/"} className="underline">
                  điều khoản
                </Link>{" "}
                của chúng tôi đề ra nhằm đảm bảo quyền lợi của người mua và
                người bán!
              </label>
            </div>
