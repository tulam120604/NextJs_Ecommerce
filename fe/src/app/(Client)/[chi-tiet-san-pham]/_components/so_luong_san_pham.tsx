        ...
        variants_attribute.filter((attr: any) => {
          ...
          attr.attribute == item &&
            attr?.value_variants.filter((s: any) => {
              s.name_variant && s.name_variant.trim() !== ""
                ? setValue_variant(attr.value_variants)
                : (setQuantity_attributes(s.stock_variant),
                  set_price_attr(s.price_variant),
                  setValue_variant(""));
            });
        });
    ...
    <div>
      ...
      {data_Item_Detail?.variant?.variants && (
        <div className="flex flex-col gap-y-4 mb-1">
          <>
            <div
              className="flex flex-wrap items-center gap-x-4 *:relative *:border *:bg-transparent *:border-gray-700 
          *:px-3 *:py-1 *:text-sm *:rounded"
            >
              {variants_attribute?.map(
                (item: any) =>
                ...
                ${name_attribute == item?.attribute && "!border-[#0A68FF]"}`}
                        key={Math.random()}
                        onClick={() =>
                          handle_attributes("Color", item?.attribute)
                        }
                      >
                        {item?.attribute}
      ...
      <div className={data_Item_Detail?.variant?.variants && "relative top-3"}>
        ...
        {/* add cart */}
        <div
          className="flex items-center gap-x-5 mt-4 font-medium lg:text-base mb:text-sm *:duration-300 
        *:bg-white *:dark:bg-[#0F1629]"
        >
          <button
            onClick={() => add_To_Cart_or_Checkout_order("add_cart")}
            className="hover:bg-gray-200 dark:hover:bg-black text-[#0A68FF] border 
            border-[#0A68FF] duration-200 lg:w-[128px] lg:h-[40px] w-[100px] h-[30px] grid 
            place-items-center rounded-md text-xs lg:text-sm"
          >
            Thêm vào giỏ
          </button>
