...
const Mo_ta_san_pham = ({ dataProps }: any) => {
  const description = String(dataProps?.data?.des_product || "");
      ...
      <div className="flex flex-col max-w-[1440px] *:rounded-lg">
        ...
        <div className="p-4 bg-gray-100 dark:bg-[#0F1629]">
          ...
          <div
            ...
            className="show_description my-4 text-sm"
          ></div>
        </div>
        {/* detail comment */}
        <section className="show_review mt-8 p-4 bg-gray-100 dark:bg-[#0F1629]">
          <span className="text-lg flex gap-x-2">
            ...
            <p>
              ...
              {dataProps?.data_feedback?.data_feedback
                ? dataProps?.data_feedback?.data_feedback?.totalDocs
                : 0}
              )
            </p>
          </span>
          <div className="flex flex-col text-sm opacity-80 leading-[21px] gap-y-4 py-4">
            {/* content comment 1 */}
            {dataProps?.data_feedback?.data_feedback &&
              dataProps?.data_feedback?.data_feedback?.docs?.map(
                (item: any) => (
                  <div key={item?._id} className="border-t lg:p-6 mb:p-5">
                        ...
                        <div>
                          <div className="flex items-center text-base text-[#1A1E26] gap-x-4 whitespace-nowrap">
                            ...
                            <span className="flex gap-x-2">
                              <p>{item?.item_order?.color_item}</p>
                              <p>{item?.item_order?.size_attribute_item}</p>
                            </span>
                          </div>
                          <span className="text-sm font-light block lg:hidden mt-1">
                    ...
                    {/* text comment */}
                    <p className="text-[#1A1E26] text-sm">
            ...
            {/*btn show more */}
            {dataProps?.data_feedback?.data_feedback &&
            dataProps?.data_feedback?.data_feedback?.totalDocs > 0 ? (
