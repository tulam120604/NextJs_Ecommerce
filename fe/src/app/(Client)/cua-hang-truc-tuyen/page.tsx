        ...
        <>
          ...
          <div className="lg:flex gap-x-10">
            ...
            <section className="w-[340px] rounded p-4 bg-gradient-to-r from-[#564F54] to-[#2C2F54] h-[120px] flex gap-x-5">
              <Image
                className="rounded-full border-4 border-gray-500 w-[70px] h-[70px]"
                width={70}
                height={70}
              ...
              <div className="flex flex-col gap-y-2">
                <span className="text-gray-100 text-lg lg:text-xl">
                  {data?.data?.user_name}
                </span>
            ...
            <section className="flex flex-col gap-y-3 pt-2 text-sm *:flex *:items-center *:gap-x-2">
              <span>
                Sản phẩm :{" "}
                <p className="text-red-500">{data_item?.data?.totalDocs}</p>
              </span>
              <span>
                Đánh giá : <p className="text-red-500">4.8 (1000 đánh giá)</p>
              </span>
            </section>
          </div>
          {/* items shop */}
          <div className="py-7">
            ...
            <div className="mx-auto py-6">
              ...
              {data_item?.data?.totalPages > 1 && (
                <Paginate_item
                  totalPages={data_item?.data?.totalPages}
                  page={data_item?.data?.page}
                />
              )}
            </div>
          </div>
