...
const Page = () => {
  ...
  return (
    <div >
      <div className='flex flex-nowrap overflow-x-scroll hidden_scroll_x items-center *:bg-none *:w-[200px] *:py-3 bg-[#F2F7FF] *:border-b-2 *:border-[#F2F7FF]'>
        ...
        <button className='hover:border-gray-900'>Đang chuẩn bị hàng({data?.data?.totalItems && data?.data?.totalItems2})</button>
        <button className='hover:border-gray-900'>Đang vận chuyển({data?.data?.totalItems && data?.data?.totalItems3})</button>
        <button className='hover:border-gray-900'>Giao thành công({data?.data?.totalItems && data?.data?.totalItems4})</button>
        <button className='hover:border-gray-900'>Đã hủy({data?.data?.totalItems && data?.data?.totalItems5})</button>
      </div>
      {
        data ?
          data?.data?.data_order?.docs?.map((item: any) =>
            <div className='shadow py-2 mb-6 px-4 lg:px-8' key={item?._id}>
              <span className='border-b px-1 py-2 text-sm'>{status_order(item?.status_item_order)}</span>
              <DataTable data={item?.items_order} columns={columns} />
              <div key={+item?._id + Math.random()} className='flex justify-end'>
                {
                  (+item?.status_item_order === 5) ?
