  ...
  return (
    <>
      ...
      <div
        ...
        } bg-gray-100 dark:bg-[#0F1629] w-screen max-h-[90vh] border rounded-t-xl 
      text-center px-2 duration-300 fixed left-0`}
      >
        <section className="py-4 w-screen relative">
          <span className="text-lg opacity-90">Danh mục sản phẩm</span>
          <button
            ...
            {data?.data?.length > 0 ? (
              data?.data?.map((item: any) => (
                <Link
                  ...
                  onClick={() => handle_popup_catalog_product(false)}
                  className="flex flex-col items-center rounded-lg hover:bg-gray-200 duration-200 gap-3"
                >
                  <Image
                    width={100}
                    height={100}
                    src={item?.category_img}
                    className="w-16 h-[85px]"
                    alt="Loading..."
                  />
                  <span className="text-xs !font-light opacity-90 line-clamp-2 whitespace-normal">
                    {item?.category_name}
                  </span>
                </Link>
              ))
