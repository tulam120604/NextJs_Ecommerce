...
export default function Catalog_product_mobile({ propsData }: any) {
  ...
  return (
    <>
      {
        popup_catalog_product && <button className='bg-[#33333366] h-full z-[-1] w-full fixed top-0 left-0'
          onClick={() => handle_popup_catalog_product(false)} />
      }
      <div className={`${popup_catalog_product ? 'translate-y-0 bottom-14' : 'translate-y-full -bottom-10'} bg-white w-screen max-h-[90vh] border rounded-t-xl 
      text-center px-2 duration-300 fixed left-0`}>
        <section className='py-4 w-screen relative'>
          ...
          <button className='absolute right-6' onClick={() => handle_popup_catalog_product(false)}>
            <X />
          </button>
        </section>
        ...
        {/* list category product */}
        {
          isError ?
            ...
            isLoading ?
              ...
              <div className='grid grid-cols-4 gap-3 mt-4 overflow-y-auto py-4'>
                {
                  data?.data?.length > 0 ?
                    data?.data?.map((item: any) => (
                      <Link key={item?._id} href={`/san-pham/${item?._id}`} onClick={() => handle_popup_catalog_product(false)}
                        className='flex flex-col items-center rounded-lg hover:bg-gray-200 duration-200 gap-2'>
                        <Image width={100} height={100} src={item?.category_img} className='w-20 h-[65px] mix-blend-darken'
                          alt='Loading...' />
