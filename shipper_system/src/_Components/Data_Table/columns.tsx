import { I_ItemOrder, IColumns } from "@/Types/Type";
import { ColumnDef } from "@tanstack/react-table";
import { CircleCheck } from "lucide-react";
import { Link } from "react-router-dom";

function status_order(item: number | string) {
    switch (+item) {
        case 1:
...
export const columns: ColumnDef<IColumns>[] = [
    {
        cell: ({ row }) => (
            row?.original?.items_order?.map((item: I_ItemOrder) => {
                return (<div key={item?.product_id?._id} className='flex items-center gap-x-4 mt-2'>
                    <img width={70} height={70} className='h-[70px] rounded border' src={item?.product_id?.gallery[0]} alt='Loading...' />
    ...
    {
        cell: ({ row }) =>
            <Link to={`shop?id=${row?.original?._id}`} className='hover:text-sky-600 text-sky-500 underline duration-100'>Chi tiết</Link>,
        'header': "Thao tác"
    }
]