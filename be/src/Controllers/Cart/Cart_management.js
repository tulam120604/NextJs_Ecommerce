...
export async function Add_To_Cart(req, res) {
  try {
    ...
    if (data_item.variant) {
      const check_name_variant = data_item.variant.variants.find(
        (value) => value.attribute === attribute
      );
      const check_value_variant = check_name_variant.value_variants.find(
        (item) =>
          (item?.name_variant?.trim() ? item?.name_variant : undefined) ===
          name_variant
      );
    ...
    let price_item =
      price_item_attr > 0 ? price_item_attr : data_item?.price_product;
    let name_varriant;
    let value_varriant;
    if (data_item.variant) {
      const varr = data_item.variant.variants.find(
        (value) => value.attribute === attribute
      );
      if (varr) {
        ...
        if (data_cart.items[i].product_id == product_id) {
          if (data_cart.items[i].name_varriant == attribute) {
            if (data_cart.items[i].value_varriant == name_variant) {
              data_cart.items[i].quantity =
                data_cart.items[i].quantity + quantity;
              data_cart.items[i].total_price_item =
                price_item * data_cart.items[i].quantity;
  ...
  try {
    ...
    for (let i = 0; i < data_user_Cart.items.length; i++) {
      if (data_user_Cart.items[i].product_id == product_id) {
        if (
          data_user_Cart.items[i].name_varriant == attribute &&
          data_user_Cart.items[i].value_varriant == name_variant
        ) {
          data_user_Cart.items[i].quantity++;
          data_user_Cart.items[i].total_price_item =
            data_user_Cart.items[i].price_item *
            data_user_Cart.items[i].quantity;
        }
      }
    }
...
export async function remove_item_cart(req, res) {
  ...
  try {
    ...
    data_cart.items = data_cart.items.filter(
      (id_item) => id_item._id.toString() !== item_id
    );
...
export async function remove_all_item_cart(req, res) {
  ...
  try {
    ...
    data_cart.items = data_cart.items.filter((item) => !item.status_checked);
    const data = await data_cart.save();
    return res.status(StatusCodes.OK).json({
      message: "Done remove!",
      data,
