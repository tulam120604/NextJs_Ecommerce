...
export async function save_item_order(
  user_id,
  items_order,
  infor_user,
  notes_order,
  ...
  let total_price_order_amount = 0;
  ...
  for (let i of items_order) {
    ...
    let check_group_item_order_by_seller = group_items_order_by_seller.find(
      (a) => a.id_shop === id_seller
    );
  ...
  const result = await Promise.all(promise_order);
...
export async function create_Order(req, res) {
  ...
  try {
    const result = await save_item_order(
      user_id,
      items_order,
      infor_user,
      notes_order,
...
export async function buy_again(req, res) {
  ...
  try {
    ...
    const item_order = await Orders.create({
      user_id,
      items_order,
      infor_user,
    });
...
export async function view_detail_item_order(req, res) {
  try {
    ...
    const data_item = data_item_orders.items_order.filter(
      (item) => item._id.toString() === id_item_order.toString()
    );
    return res.status(StatusCodes.OK).json({
      message: "OK",
      data_item,
